...
export async function listColleges(req: Request, res: Response) {
  ...
  const maxFee = parseOptionalFloat(req.query.maxFee);
  const page = parsePositiveInt(req.query.page, 1);
  const limit = parsePositiveInt(req.query.limit, 12, 50);
  const skip = (page - 1) * limit;

  const rawCategory = normalizeString(req.query.category) ?? "OPEN";
...
// ─── Compare Colleges ─────────────────────────────────────────────────────────
export async function compareColleges(req: Request, res: Response) {
  ...
  return res.json({ colleges: colleges.map(mapCollege) });
}
